...
根据公式$z = x\_1 w\_1+x\_2 w\_2 + b$，神经网络想学习w1和w2，但是数值范围问题导致神经网络来说很难“理解”。图5-5展示了标准化前后的情况损失函数值的等高图，意思是地理位置和房屋面积取不同的值时，作为组合来计算损失函数值时，形成的类似地图的等高图，见图5-5，左侧为标准化前，右侧为标准化后。
...
$$x\_{new}={x-x\_{min} \over x\_{max} - x\_{min}} \tag{1}$$

* 平均值标准化，将数据映射到\[-1,1\]区间

$$x\_{new} = {x - \bar{x} \over x\_{max} - x\_{min}} \tag{2}$$

* 对数转换 $$x\_{new}=log\(x\_i\) \tag{3}$$
* 反余切转换 $$x\_{new}=atan\(x\_i\) \cdot 2/π \tag{4}$$
* Z-Score法

把每个特征值中的所有数据，变成平均值为0，标准差为1的数据，最后为正态分布。Z-Score规范化（标准差标准化 / 零均值标准化，其中std是标准差）：

$$x\_{new} = \(x\_i - \bar{x}\)／std \tag{5}$$

* 中心化，平均值为0，无标准差要求

$$x\_{new} = x\_i - \bar{x} \tag{6}$$

* 比例法，要求数据全是正值

$$ x\_{new} = x\_i / \sum\_{i=1}^n{x\_i} \tag{7} $$
